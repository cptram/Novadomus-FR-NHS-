...
def draw_credential_card(c, ox, oy, spec, palette=None):
    ...
    # facility / holder name (wrapped)
    ty = oy + H - pad - 13 * mm
    for line in _wrap(c, spec.get("title", ""), "Helvetica-Bold", 12, W - 2 * pad - 2 * mm)[:2]:
    ...
    if spec.get("subtitle"):
        ...
        c.drawString(ox + pad, ty, spec["subtitle"])

    # ID code — prominent, gold
    c.setFillColor(col("gold_lite"))
    c.setFont("Helvetica-Bold", 13)
    c.drawString(ox + pad, oy + 20 * mm, spec.get("id_code", ""))
